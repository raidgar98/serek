...
void serek_benchmark_controller::serialize(const model_t& in, std::string& out) const
{
	out = serek::serial::json::serialize(in);
...

			{
				...
				{\it Jmeter}\texttrademark\cite{jmeter} jest oprogramowaniem stworzonym przez fundację {\it Apache}, napisaną w języku {\it Java}. jej przeznaczeniem
				...
				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/main_pane.png}
					\caption{Widok ogólnej konfiguracji całego testu}
					\medskip
				\end{screenshot}
				...
				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/thread_group_config.png}
					\caption{Konfiguracja grupy wątków}
					\medskip
				\end{screenshot}
				...
				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/variables.png}
					\caption{Konfiguracja zmiennych}
					\medskip
				\end{screenshot}
				...
				tekstu pomocy. Tajemniczy zapis w kolumnie wartości, jest spowodowany sposobem uruchamiania testów wydajnościowych za pomocą programu {\it Jmeter}\texttrademark,
				...
				nadpisania domyślnych wartości poprzez dodanie odpowiednich parametrów do polecenia (za pomocą flagi {\it -J nazwa\_pola=nowa\_wartość}).\n

				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/jmeter_gui_warn.png}
					\caption{Ostrzeżenie dotyczące nie przeprowadzania testów wydajnościowych w trybie graficznym}
					\medskip
				\end{screenshot}
				...
				zostało zdefiniowane jako plik {\it CSV}, którego stokenizowana każda linijka jest przekazywana do każdego wątku niezależnie od siebie. Dzięki takiemu rozwiązaniu,
				każda biblioteka zostanie uczciwie przetestowana takim samym zestawem danych.\n

				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/csv_config.png}
					\caption{Konfiguracja punktu źródła danych do zapytań}
					\medskip
				\end{screenshot}

				Ostatnimi miejscami do uzupełnienia jest globalna konfiguracja klienta {\it HTTP} oraz sprecyzowanie ścieżek pod jakimi dostępne są konkretne biblioteki.

				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/http_serek_config.png}
					\caption{Konfiguracja klienta {\it HTTP} dla biblioteki {\it serek}}
					\medskip
				\end{screenshot}
				...
				\begin{screenshot}[H]
					\centering
					\includegraphics[width=15cm,keepaspectratio=true]{./img/jmeter_ss/aggregate_report.png}
					\caption{Szybki raport wygenerowany przez aplikację}
					\medskip
				\end{screenshot}
			}
